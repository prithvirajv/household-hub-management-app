...
function normalizePayeeForMatch(value) {
...
function isDuplicateTransaction(candidate, existingTransactions) {
  const candidatePayee = normalizePayeeForMatch(candidate.payee);
  ...
  return (existingTransactions || []).some((transaction) =>
    ...
    normalizePayeeForMatch(transaction.payee) === candidatePayee &&
