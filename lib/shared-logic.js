...
// cent regardless of split type or rounding. Reuses splitAmountEvenly for
// "equal" and splitBillByPercentages for "percentage"; "exact" just reads
// each participant's typed amount.
function computeBillSplitAmounts(splitType, totalAmount, participants) {
  ...
  dataRows.forEach((row) => {
    ...
    if (debitIndex !== -1) {
      ...
      if (Number.isFinite(debitValue) && debitValue > 0) {
        ...
        // A separate Debit/Credit column pair (Citi and similar) needs the
        // same payoff-skipping/refund-keeping treatment as the single signed
        // Amount column above - a Credit row is either an autopay/payment
        // (skip) or a genuine refund (keep, as a negative amount). Some
        // exports store the Credit value as already-negative, others as a
        // plain positive magnitude - normalize to negative either way, since
        // that's this parser's convention for money coming back.
        const creditValue = parseCsvAmount(row[creditIndex]);
        if (Number.isFinite(creditValue) && creditValue !== 0 && !STATEMENT_PAYMENT_DESCRIPTION.test(payee)) {
      ...
      if (!Number.isFinite(signedValue) || signedValue === 0) {
        ...
        // down the card, not a purchase or a refund of one - skip it just
        // like parseCreditCardStatementText does. Everything else keeps its
        // real sign: positive stays a purchase, negative stays a refund/
        // credit (nets against the original purchase's spend automatically).
        amount = STATEMENT_PAYMENT_DESCRIPTION.test(payee) ? NaN : signedValue;
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    splitAmountEvenly, splitBillByPercentages, normalizePersonName, netBalancesByPerson, computeBillSplitAmounts, settleUpPersonIous, isValidEmail,
