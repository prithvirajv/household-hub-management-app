...
const PAYEE_FUZZY_MATCH_PREFIX_LENGTH = 6;
...
const REFUND_FUZZY_MATCH_DATE_WINDOW_DAYS = 120;
...
// share a payee/amount, but not a real order id), so it's tried first and the
// fuzzy fallback only runs when the candidate has no orderNumber to go on.
function refundMatch(candidate, pool) {
  return orderRefundMatch(candidate, pool) || (candidate.orderNumber ? null : refundFuzzyMatch(candidate, pool));
}
