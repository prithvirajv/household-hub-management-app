...
// requiring a fresh pick every time: an exact-normalized-payee match wins
// first (the overwhelming majority of repeat payees - the same "SAWNEE EMC
// Bill Payment" every month), falling back to the same fuzzy same-merchant
// heuristic refundMatch already uses for a near-miss spelling/format
// variant ("TARGET.COM" vs "TARGET STORE 1147"). The single most *recently*
// categorized transaction for a payee wins outright - not whichever line has
// the most historical hits - so a household that changed how it categorizes
// a payee (moved "Netflix" from Entertainment to a new Subscriptions line)
// gets the newer choice immediately, rather than staying stuck suggesting
// the old line until it eventually outnumbers the new one. Only
// already-categorized transactions (a real lineId) count as history - an
// unassigned or still-pending row teaches this nothing.
function suggestSubcategoryFromHistory(payee, transactions) {
  const targetKey = normalizeForPayeeMatch(payee);
  if (!targetKey) return null;
  const latestByPayee = new Map();
  (transactions || []).forEach((transaction) => {
    if (!transaction.lineId) return;
    const key = normalizeForPayeeMatch(transaction.payee);
    if (!key) return;
    const date = String(transaction.date || "");
    const existing = latestByPayee.get(key);
    if (!existing || date > existing.date) latestByPayee.set(key, { rawPayee: transaction.payee, lineId: transaction.lineId, date });
  });
  const exact = latestByPayee.get(targetKey);
  if (exact) return exact.lineId;
  let bestFuzzy = null;
  latestByPayee.forEach((entry) => {
    if (!payeesFuzzyMatch(payee, entry.rawPayee)) return;
    if (!bestFuzzy || entry.date > bestFuzzy.date) bestFuzzy = entry;
  });
  return bestFuzzy ? bestFuzzy.lineId : null;
}
