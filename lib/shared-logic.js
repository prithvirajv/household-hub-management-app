...
// A CSV row's payee is the bank's raw statement description ("ROCKET
// MORTGAGE DES:LOAN ID:8698964 INDN:... WEB"), while the same charge in the
// ledger may carry a short, hand-typed or recurring-bill name ("Rocket
// Mortgage"). An exact string match misses this, so reuse the same
// mutual-substring heuristic as matchAccountByFilename instead of requiring
// equality.
function payeesLikelyMatch(left, right) {
  const normalizedLeft = normalizeForAccountMatch(left);
  const normalizedRight = normalizeForAccountMatch(right);
  return Boolean(normalizedLeft) && Boolean(normalizedRight) &&
    (normalizedLeft.includes(normalizedRight) || normalizedRight.includes(normalizedLeft));
}

// A CSV re-imported over an overlapping date range (or a bank stream item
// that was already accepted into the ledger) reproduces the same
// amount/payee, but the date can drift a day or two — a pending charge
// often posts a couple days later/earlier than it first appeared. Flag
// those matches so the household can see it before approving, rather than
// silently double-counting the spend.
function isDuplicateTransaction(candidate, existingTransactions) {
  ...
  return (existingTransactions || []).some((transaction) =>
    ...
    payeesLikelyMatch(transaction.payee, candidate.payee) &&
    daysBetweenDateKeys(transaction.date, candidate.date) <= DUPLICATE_TRANSACTION_DATE_TOLERANCE_DAYS
  );
}
