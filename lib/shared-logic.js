  ...
  dataRows.forEach((row) => {
    ...
    if (debitIndex !== -1) {
      ...
      if (Number.isFinite(debitValue) && debitValue > 0) {
        ...
        // same payoff-skipping treatment as the single signed Amount column
        // below - a Credit row is either an autopay/payment (skip) or real
        // money in (keep, as a negative amount, flagged as a deposit so Bank
        // Stream can label it). Some exports store the Credit value as
    ...
    if (date && payee && Number.isFinite(amount) && amount !== 0) {
      results.push(isDeposit ? { date, payee, amount, isDeposit: true } : { date, payee, amount });
    }
  });
  return results;
}
