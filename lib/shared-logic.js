...
// The authoritative due date for a birthday/anniversary reminder. Deliberately
// ignores any previously-stored notifyAt (which may have been computed from a
// literal entered date, e.g. a birth year decades in the past) and always
// derives it fresh from the next upcoming occurrence minus reminderDays.
...
// every payday that has actually passed, not only the first.
...
function accountBalance(accountId, { accounts, transactions, paychecks, transfers }, referenceDateKey) {
  ...
  const transfersIn = (transfers || [])
    .filter((transfer) => transfer.toAccountId === accountId)
    .reduce((sum, transfer) => sum + Number(transfer.amount || 0), 0);
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    paycheckOccurrencesSince, accountBalance, accountsWithBalances,
