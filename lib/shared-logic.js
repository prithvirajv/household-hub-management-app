...
// brokerage account. A holding created via the bulk/manage-holdings dialog
// carries an explicit groupId/groupName; a legacy solo holding (added one
// at a time before grouping existed) has neither, so it falls back to being
// its own single-holding group keyed by its own id - every stock-class
// asset ends up in exactly one group, none dropped silently.
function groupStockHoldings(assets) {
  const groups = [];
  const byKey = new Map();
  assets.filter((item) => item.assetClass === "stock").forEach((item) => {
