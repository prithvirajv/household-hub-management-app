...
// paycheck is ended. An optional skippedDates list (individual date keys)
// excludes just those specific occurrences - e.g. a paycheck that normally
// repeats but was not actually received one month - without affecting any
...
function paycheckOccurrencesSince(paycheck, referenceDateKey) {
  ...
  if (recurrence === "once" || recurrence === "bonus") {
    ...
    if (recurrence === "weekly") rawCount = Math.floor((reference - anchor) / (7 * dayMs)) + 1;
    else if (recurrence === "biweekly") rawCount = Math.floor((reference - anchor) / (14 * dayMs)) + 1;
    else if (recurrence === "monthly") {
      const months = (reference.getFullYear() - anchor.getFullYear()) * 12 + (reference.getMonth() - anchor.getMonth());
      rawCount = months + (reference.getDate() >= anchor.getDate() ? 1 : 0);
    } else rawCount = 1;
  }
  const skippedCount = (paycheck.skippedDates || []).filter((date) => date >= paycheck.date && date <= effectiveReferenceKey).length;
  return Math.max(0, rawCount - skippedCount);
}
...
// Same as paycheckAllOccurrenceDatesInRange but excludes any date the user
// marked as skipped (a single missed/not-received payday), which is what
// income totals should use. The Paycheck/Income page itself still needs the
// unfiltered list so a skipped date can be shown (and un-skipped) rather than
// silently vanishing.
function paycheckOccurrenceDatesInRange(paycheck, rangeStartKey, rangeEndKey) {
  const skipped = paycheck?.skippedDates || [];
  return paycheckAllOccurrenceDatesInRange(paycheck, rangeStartKey, rangeEndKey).filter((date) => !skipped.includes(date));
}
...
// needs its own transaction row a user can see, edit, or delete.
...
function accountBalance(accountId, { accounts, transactions, paychecks, transfers }, referenceDateKey) {
  ...
  const deposits = (paychecks || [])
    .filter((paycheck) => paycheck.depositAccountId === accountId)
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    paycheckOccurrencesSince, paycheckOccurrencesInRange, paycheckOccurrenceDatesInRange, paycheckAllOccurrenceDatesInRange, recurringExpenseOccurrenceDates, accountBalance, accountsWithBalances,
