...
// DCU (and other credit unions) export a single signed Amount column plus a
// separate "Transaction Type" column holding the literal label DEBIT/CREDIT
// per row - an unambiguous, authoritative signal for which way the money
// moved, unlike guessing from the file's positive/negative row counts or
// payee text (see isCreditCardStyle below). Checked for first.
...
function parseBankCsvTransactions(text) {
  ...
  const transactionTypeIndex = header.findIndex((cell) => CSV_TRANSACTION_TYPE_HEADERS.includes(cell));
  const dataRows = rows.slice(headerIndex + 1);

  let isCreditCardStyle = false;
  // A checking account's own outgoing payment that funds a *different*
  // account's autopay (e.g. this checking account's "CITI AUTOPAY PAYMENT"
  // withdrawal, paying down an external Citi card) matches the same
  // payment-description wording the heuristic below uses to spot a credit
  // card's own payoff row, so it was misfiring on ordinary checking exports
  // whenever the household happened to pay other cards by ACH from this
  // account - one bad row-count skew or matching description was enough to
  // flip the whole file to the wrong (credit-card) sign convention, stored
  // amounts as raw exactly like the CSV, and every deposit/withdrawal in
  // that file displayed backwards. The explicit per-row type label below is
  // authoritative and never needs this guess at all, so it's skipped
  // whenever that column exists.
  if (debitIndex === -1 && transactionTypeIndex === -1 && amountIndex !== -1) {
  ...
  dataRows.forEach((row) => {
    ...
    if (transactionTypeIndex !== -1) {
      const typeLabel = String(row[transactionTypeIndex] || "").trim().toLowerCase();
      const magnitude = Math.abs(parseCsvAmount(row[amountIndex]));
      if (Number.isFinite(magnitude) && magnitude !== 0 && (typeLabel === "debit" || typeLabel === "credit")) {
        if (typeLabel === "debit") {
          amount = magnitude;
        } else {
          amount = -magnitude;
...
// --- Calendar export/import (ICS + CSV) ---
//
// Hand-rolled rather than a library: the app only ever needs to round-trip
// its own small, fixed set of recurrence enums (see
// CHORE_MONTH_STEP_BY_RECURRENCE above), so a full RRULE engine is more than
// this needs. Chores/reminders use a floating (no timezone/offset) local
// DATE-TIME for DTSTART, matching how the rest of the app already stores
// wall-clock times with no explicit zone; birthdays/anniversaries use a
// DATE-only DTSTART with FREQ=YEARLY, the conventional way calendar apps
// represent an annual date.

function icsEscapeText(text) {
  return String(text || "").replace(/\\/g, "\\\\").replace(/;/g, "\\;").replace(/,/g, "\\,").replace(/\n/g, "\\n");
}

function icsUnescapeText(text) {
  return String(text || "").replace(/\\n/g, "\n").replace(/\\,/g, ",").replace(/\\;/g, ";").replace(/\\\\/g, "\\");
}

// RFC 5545 requires folding lines longer than 75 octets; folded per
// character (not per UTF-8 octet) since this app's titles/locations are
// short enough that the distinction never matters in practice.
...
function icsDateOnly(dateKeyValue) {
  return String(dateKeyValue || "").replace(/-/g, "");
}

function icsDateTimeFloating(dateKeyValue, time) {
  const [hour, minute] = String(time || "09:00").split(":");
  return `${icsDateOnly(dateKeyValue)}T${String(hour).padStart(2, "0")}${String(minute).padStart(2, "0")}00`;
}
...
// Chores repeat on the same month-multiple grid the rest of the app already
// uses (CHORE_MONTH_STEP_BY_RECURRENCE), so RRULE mirrors that exactly
// rather than using a calendar-YEARLY rule for the "yearly" chore cadence -
// consistent with how nextPendingChoreOccurrence itself walks the grid.
function choreIcsRRule(chore) {
  const recurrence = chore.recurrence || "once";
  let rule = null;
...
function reminderIcsRRule(recurrence) {
...
function calendarItemToIcsLines(kind, item) {
  const lines = ["BEGIN:VEVENT"];
  const id = item.id || "";
  lines.push(`UID:${kind}-${id}@familyloop`);
  lines.push(`DTSTAMP:${icsStampNow()}`);
  lines.push(foldIcsLine(`SUMMARY:${icsEscapeText(item.title)}`));
  if (kind === "chore") {
    lines.push(`DTSTART:${icsDateTimeFloating(item.startDate, item.time)}`);
    const rrule = choreIcsRRule(item);
    if (rrule) lines.push(`RRULE:${rrule}`);
    if (item.location) lines.push(foldIcsLine(`LOCATION:${icsEscapeText(item.location)}`));
    lines.push("X-FAMILYLOOP-KIND:chore");
    lines.push("X-FAMILYLOOP-TYPE:chore");
    lines.push(`X-FAMILYLOOP-RECURRENCE:${item.recurrence || "once"}`);
    if (item.endDate) lines.push(`X-FAMILYLOOP-ENDDATE:${item.endDate}`);
  } else if (item.annual) {
    lines.push(`DTSTART;VALUE=DATE:${icsDateOnly(item.date)}`);
    lines.push("RRULE:FREQ=YEARLY");
    lines.push("X-FAMILYLOOP-KIND:event");
    lines.push(`X-FAMILYLOOP-TYPE:${item.type}`);
    lines.push("X-FAMILYLOOP-RECURRENCE:yearly");
    if (item.reminderDays !== undefined && item.reminderDays !== null) lines.push(`X-FAMILYLOOP-REMINDERDAYS:${item.reminderDays}`);
  } else {
    lines.push(`DTSTART:${icsDateTimeFloating(item.date, item.dateTime?.slice(11, 16))}`);
    const rrule = reminderIcsRRule(item.recurrence);
    if (rrule) lines.push(`RRULE:${rrule}`);
    if (item.location) lines.push(foldIcsLine(`LOCATION:${icsEscapeText(item.location)}`));
    lines.push("X-FAMILYLOOP-KIND:event");
    lines.push(`X-FAMILYLOOP-TYPE:${item.type || "reminder"}`);
    lines.push(`X-FAMILYLOOP-RECURRENCE:${item.recurrence || "once"}`);
  }
  ...
  lines.push(`X-FAMILYLOOP-ID:${id}`);
  lines.push("END:VEVENT");
  return lines;
}

function buildCalendarIcs(events, chores) {
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//FamilyLoop//Calendar//EN", "CALSCALE:GREGORIAN"];
  (chores || []).forEach((chore) => lines.push(...calendarItemToIcsLines("chore", chore)));
  (events || []).forEach((event) => lines.push(...calendarItemToIcsLines("event", event)));
  lines.push("END:VCALENDAR");
  return lines.join("\r\n");
}

// Unfolds continuation lines (RFC 5545: a line starting with a space or tab
// is a continuation of the previous line), then groups BEGIN:VEVENT/
// END:VEVENT blocks into raw {summary, dtstart:{date,time}, rrule, location,
// xProps} objects - no interpretation of what the event "means" happens
// here, that's icsEventsToCalendarDrafts's job.
function parseIcsText(text) {
  ...
  unfolded.forEach((line) => {
    if (line === "BEGIN:VEVENT") {
      current = { summary: "", dtstart: null, rrule: null, location: "", xProps: {} };
      return;
    }
    if (line === "END:VEVENT") {
    ...
    if (!match) return;
    const [, rawName, rawParams, rawValue] = match;
    const name = rawName.toUpperCase();
    if (name === "SUMMARY") current.summary = icsUnescapeText(rawValue);
    else if (name === "DTSTART") {
      const isDateOnly = /VALUE=DATE(?!-TIME)/i.test(rawParams || "") || /^\d{8}$/.test(rawValue);
      const digits = rawValue.replace(/Z$/i, "");
      const date = `${digits.slice(0, 4)}-${digits.slice(4, 6)}-${digits.slice(6, 8)}`;
      const time = isDateOnly || digits.length < 13 ? null : `${digits.slice(9, 11)}:${digits.slice(11, 13)}`;
      current.dtstart = { date, time };
    } else if (name === "RRULE") {
      const parts = Object.fromEntries(rawValue.split(";").map((pair) => pair.split("=")));
      current.rrule = { freq: parts.FREQ || "", interval: Number(parts.INTERVAL || 1) };
    } else if (name === "LOCATION") current.location = icsUnescapeText(rawValue);
    else if (name.startsWith("X-FAMILYLOOP-")) current.xProps[name] = icsUnescapeText(rawValue);
  });
  return events;
}

// Maps parsed VEVENTs into a flat draft shape ready for insertion into
// state.calendar. When X-FAMILYLOOP-* markers are present (this app's own
// export, being re-imported) the original kind/type/recurrence round-trips
// exactly. Otherwise (a real external .ics, e.g. from Google/Apple/Outlook)
// this falls back to a safe, generic mapping - everything becomes a
// "reminder" rather than guessing at chore/birthday/anniversary semantics
// that an external calendar has no reliable way to signal.
function icsEventsToCalendarDrafts(icsEvents) {
  return (icsEvents || [])
    .filter((icsEvent) => icsEvent.dtstart && icsEvent.dtstart.date && icsEvent.summary)
    .map((icsEvent) => {
      const assigneeKeys = icsEvent.xProps["X-FAMILYLOOP-ASSIGNEES"] ? icsEvent.xProps["X-FAMILYLOOP-ASSIGNEES"].split("|").filter(Boolean) : [];
      if (icsEvent.xProps["X-FAMILYLOOP-KIND"]) {
        return {
          kind: icsEvent.xProps["X-FAMILYLOOP-KIND"],
          type: icsEvent.xProps["X-FAMILYLOOP-TYPE"] || icsEvent.xProps["X-FAMILYLOOP-KIND"],
          title: icsEvent.summary,
          date: icsEvent.dtstart.date,
          time: icsEvent.dtstart.time || "09:00",
          recurrence: icsEvent.xProps["X-FAMILYLOOP-RECURRENCE"] || "once",
          endDate: icsEvent.xProps["X-FAMILYLOOP-ENDDATE"] || "",
          location: icsEvent.location || "",
          assigneeKeys,
          reminderDays: icsEvent.xProps["X-FAMILYLOOP-REMINDERDAYS"] !== undefined ? Number(icsEvent.xProps["X-FAMILYLOOP-REMINDERDAYS"]) : undefined
        };
      }
...
function calendarCsvField(value) {
  const text = String(value ?? "");
  return /[",\n]/.test(text) ? `"${text.replace(/"/g, '""')}"` : text;
}

function buildCalendarCsv(events, chores) {
  const rows = [CALENDAR_CSV_HEADERS.join(",")];
  (chores || []).forEach((chore) => {
    rows.push([
      "chore", "chore", chore.title, chore.startDate, chore.time || "", chore.recurrence || "once", chore.endDate || "",
...
function parseCalendarCsv(text) {
  ...
  const indices = { kind: indexOf("Kind"), type: indexOf("Type"), title: indexOf("Title"), date: indexOf("Date"), time: indexOf("Time"), recurrence: indexOf("Recurrence"), endDate: indexOf("EndDate"), location: indexOf("Location"), assignees: indexOf("Assignees"), reminderDays: indexOf("ReminderDays") };
  if (indices.title === -1 || indices.date === -1) return [];
  return rows.slice(1)
    .filter((row) => row[indices.title] && row[indices.date])
    .map((row) => ({
      kind: row[indices.kind] || "event",
      type: row[indices.type] || "reminder",
      title: row[indices.title],
      date: row[indices.date],
      time: row[indices.time] || "09:00",
      recurrence: row[indices.recurrence] || "once",
      endDate: row[indices.endDate] || "",
      location: row[indices.location] || "",
      assigneeKeys: row[indices.assignees] ? row[indices.assignees].split("|").filter(Boolean) : [],
      reminderDays: indices.reminderDays !== -1 && row[indices.reminderDays] !== "" && row[indices.reminderDays] !== undefined ? Number(row[indices.reminderDays]) : undefined
    }));
}
...
if (typeof module !== "undefined" && module.exports) {
  module.exports = {
    ...
    buildCalendarIcs, parseIcsText, icsEventsToCalendarDrafts, buildCalendarCsv, parseCalendarCsv, icsEscapeText, foldIcsLine
  };
}
